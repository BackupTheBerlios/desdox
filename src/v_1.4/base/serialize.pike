...
private static string  serialize_function(function f);
        string  serialize(mixed arg,void|object oThis);
...
unserialize_object(string stream)
{
    ...
    //LOG("unserialize_object ("+stream+")");
    sscanf(stream, "%d", res);
    //    LOG(sprintf("find_object = %O",find_object));
    proxy = find_object(res);

    //    LOG("Unserialize_object("+proxy->status()+")");
    //if ( proxy->status() == PSTAT_FAIL_DELETED )
     //   return 0;
...
#if 0
	if (!functionp(res)) 
	    throw( ({ "Failed to unserialize function "+sFunctionName+
			  " in object "+master()->stupid_describe(o)+"\n",
			  backtrace() }) );
#endif
    }
    else
	throw( ({"Failed to unserialize function "+f+"\n", backtrace() }) );
    return res;
}
