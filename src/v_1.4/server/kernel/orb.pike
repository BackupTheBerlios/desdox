...
{
    array directory = ({ });
    object cont = path_to_object(dir);
    if ( objectp(cont) ) {
...
mapping(string:array(int)) query_directory(string|object url)
{
    ...
    res = ([ ]);
    if ( stringp(url) )
	cont = path_to_object(url);
    else
	cont = url;
    LOG("Inventory of: " + cont->get_identifier());
    inv = cont->get_inventory();
    for ( i = 0, sz = sizeof(inv); i < sz; i++ ) {
	if ( objectp(inv[i]) && 
	     stringp(inv[i]->get_identifier()) && 
	     strlen(inv[i]->get_identifier()) > 0 &&
	     !(inv[i]->get_object_class() & CLASS_SCRIPT) &&
	     !(inv[i]->get_object_class() & CLASS_USER) )
	{
