...
private array(int) text_extends(array(string) list, Image.TTF font)
{
...
private array(string) wrap_with_font(string from, int width, object font)
{
    ...
    array(int) charlens = text_extends(from/"", font);
    int i,sz;
    int last_pos = 0;
    int curr_len = 0;
    array(string) result = ({});
...
string get_image_map(object obj, void|mapping vars)
{
    ...
    sImageMap = "<html><body><map name=\""+obj->get_object_id()+"\">\n";
    foreach(aoInv, object graphics)
    {
        int iOclass;
		...
		MESSAGE("Module"+sMime);
            ...
            sImageMap +="  <area shape=rect coords=\""+
                x1+ ", " + y1 + ", " + (x1+w) + ", " + (y1+h) +"\" "+
		href_link_navigate(graphics, vars->prefix)+" >\n";

            oLine = Image.Font()->write(graphics->get_identifier());
            sImageMap +="  <area shape=rec coords=\""+
    ...
    sImageMap += "</map> <img src=\"/scripts/svg.svg?object="+obj->get_object_id()+
        "&amp;enc=image/jpeg\" usemap=\"#"+obj->get_object_id()+"\">\n"+
        "</body></html>";
