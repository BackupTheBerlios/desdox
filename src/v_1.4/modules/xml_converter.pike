...
#else
([ "description":  ({ THIS, "query_attribute", ({OBJ_NAME}), show }) ]), ]);
#endif
...
mapping userXML = ([ CLASS_USER: ([
    ...
    "admin":         ({ MODULE_GROUPS->lookup("admin"), "is_member", 
			    ({ THIS }), show_truefalse }),
...
static void load_module()
{
    ...
    CONV->obj = this();
    mXMLCache = ([ ]);
    add_global_event(EVENT_REQ_SAVE, save_object, PHASE_NOTIFY);
...
/**
 * Callback function when an object is saved to clear the XML Cache.
 * The converter caches the generated XML code for each object.
 *  
 * @param object caller - the calling object.
 * @param object p - the saved object.
 * @author <a href="mailto:astra@upb.de">Thomas Bopp</a>) 
 */
void save_object(object caller, object p)
{
    if ( stringp(mXMLCache[p]) )
	mXMLCache[p] = 0;
}
 ...
 *  
 * @param int t - timestamp to get a string description for.
...
string get_obj_content_rxml(object obj)
{
    if ( search(obj->query_attribute(DOC_MIME_TYPE), "txt") >= 0 ||
	 search(obj->query_attribute(DOC_MIME_TYPE), "text") >= 0 ) 
    {
	string content = "<?xml version='1.0' encoding='iso-8859-1'?><div>"+
	    obj->get_content()+"</div>";
	mapping tags = ([ "xslt": xsltTag(), ]);
	string result;
	mixed err = catch {
	    result = rxml(content, XSL->vars, tags);
 ...
 * @author Thomas Bopp (astra@upb.de) 
 */
string describe_params(array reg)
{
...
_get_annotations_thread(object obj, object user , int d, int|void active_flag)
{
	...
	"<depth>"+d+"</depth>\n"+
#ifdef RESTRICTED_NAMES
	"<subject><![CDATA["+obj->query_attribute(OBJ_DESC) +"]]></subject>"+
#else
	"<subject><![CDATA["+obj->query_attribute(OBJ_NAME) +"]]></subject>"+
#endif
...
string serialize_xml_tag(string tag, array data)
{
	...
	if ( err != 0 ) {
	    DEBUG_XML("While serializing XML:\n"+
