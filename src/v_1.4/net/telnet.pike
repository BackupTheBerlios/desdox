...
int cmd_tell(string cmd, string args)
{
	...
	target = MODULE_USERS->lookup(user);
	if ( !objectp(target) ){
...
int cmd_upgrade(string cmd, string args)
{
    ...
    // master()->upgrade(master()->programs["/classes/Bug.pike"]);
    // master()->programs["/classes/Bug.pike"]=compile_string(master()->master_read_file("/classes/Bug.pike"));
    if(res==-1)
...
static void process_command(string cmd)
{
    ...
    case MODE_CONNECT:
      ...
      break;
    case MODE_LOGIN:
	tmpUser = MODULE_USERS->lookup(cmd);
	if ( !objectp(tmpUser) )
        {
