...
#define MIME_FILE "config/mimetypes.txt"
#define CLASS_FILE "config/classtypes.txt"
 ...
 */
void register_class(string cl, string doc_class, string mime, string desc)
{
    if ( !_SECURITY->access_register_class(0, CALLER) )
	return;
...
init_mime()
{
    ...
    foreach(n->sibling, NodeXML type) {
	...
	j = 0; start = -1; tokens = ({ });
	while ( j < len ) {
	    if ( lines[i][j] == ' ' || lines[i][j] == '\t' || j == len-1 ) {
		if ( start != -1 ) {
