...
void notify(int event, mixed ... args)
{
    ...
    switch(event) {
	case EVENT_SAY:
	    if ( !(args[1]->get_object_class() & CLASS_USER) ||  
		 args[1]->get_user_object() != oUser )  
		send_message(cryptic_irc_name(args[1]) + " PRIVMSG " + 
	    ...
	    if ( args[1]->get_object_class() & CLASS_USER &&
		 args[1]->get_status() & CLIENT_FEATURES_CHAT )
		send_message(cryptic_irc_name(args[1]) + " JOIN " + 
			     channel_name(args[0])+"\r\n");
	    break;
        case EVENT_LEAVE_INVENTORY:
	    if ( args[1]->get_object_class() & CLASS_USER &&
		 args[1]->get_status() & CLIENT_FEATURES_CHAT )
		send_message(cryptic_irc_name(args[1]) + " PART " + 
			     channel_name(args[0])+"\r\n");
	    break;
        case EVENT_TELL:
...
static void connect_user(object u)
{
    ...
    u->listen_to_event(EVENT_TELL, u);    
}
...
static void user(string u)
{
    ...
    if ( !stringp(sPass) ) {
//	connect_user(_GUEST);
	object factory = get_factory(CLASS_GHOST);
	//object ghost = factory->execute( (["name":sNick, ]));
	//connect_user(ghost);
    ...
    user = MODULE_USERS->lookup(sNick);
    if ( objectp(user) && user->check_user_password_plain(sPass) ) 
    {
	if ( objectp(oUser) ) {
	    login_user(user);
	}
...
static int join_channel(object channel)
{
	...
	if ( objectp(oChannel) ) {
	    oUser->dispose_event(
		EVENT_SAY|EVENT_ENTER_INVENTORY|EVENT_LEAVE_INVENTORY, 
		oChannel);
	    oUser->dispose_event(
		EVENT_LOGIN|EVENT_LOGOUT|EVENTS_MONITORED,oChannel);
	}
	
	oUser->listen_to_event(
	    EVENT_SAY|EVENT_ENTER_INVENTORY|EVENT_LEAVE_INVENTORY, channel);
	oUser->listen_to_event(EVENT_LOGIN|EVENT_LOGOUT|EVENTS_MONITORED, 
			       channel);
    };
    return err == 0;
}
...
{
    array(object) users = channel->get_users();
...
static void who(string channel)
{
    ...
    if ( objectp(chann) ) {
	foreach(chann->get_users(), object u) {
	    if ( u->get_status() & CLIENT_FEATURES_CHAT )
		users += ({ u });
	}
    }
...
static void join(string channels) 
{
	...
	if ( join_channel(chann) ) {
	    ...
	    string msg = cryptic_irc_name(oUser)+" JOIN "+
			 channel_name(chann)+"\r\n";
	    werror("Sending join message ... "+msg);
	    send_message(msg);
	    reply_topic(chann);
	    list_users(chann);
	}
...
{	    
    ...
    send_message(cryptic_irc_name(this_user()) + " INVITE "+oUser->get_identifier() + " "+channel+"\r\n");
}
...
static void exec_response(string res)
{
    ...
    send_message(cryptic_irc_name(this_user()) + " PRIVMSG "+
...
#endif
	    sPass = msg;
	    object u = MODULE_USERS->lookup(sNick);
	    if ( u->check_user_password_plain(sPass) ) 
		connect_user(u);
	    return;
	case "DCC":
	case "dcc":
...
{
    string str;
    array inv = this_user()->get_inventory();
...
static void look()
{
    object channel = this_user()->get_environment();
    send_message(cryptic_irc_name(this_user()) + " PRIVMSG " +
...
static void download(string fname)
{
    object doc = oChannel->get_object_byname(fname);
    int port = 33333;
    string ipaddr = _Server->query_config("ip");
